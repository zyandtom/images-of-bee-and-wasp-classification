...
def compute_neural_net_loss(params, X, y, reg=0.0):
    ...
    relu = lambda x: x * (x > 0)
    u1 = relu(z1)
    z2 = np.matmul(u1, W2) + b2

    #loss
    sumz2 = np.exp(z2).sum(axis = 1)
    for i in range(N):
        loss += -np.log(np.exp(z2[i][y[i]])/sumz2[i])/N
    ...
    #############################################################################
    #initial values
    sigma2 = np.zeros(np.shape(z2))

    #error matrix of output and accumulated db2
    C = len(z2[0])   #number of class
    for i in range(C):
        for j in range(N):
            if y[j] == i:
                sigma2[j][i] = np.exp(z2[j][i])/sumz2[j] - 1
            else:
                sigma2[j][i] = np.exp(z2[j][i])/sumz2[j]
    sigma2 /= N
    db2 = sigma2.sum(axis = 0)

    #compute the dW2
    dW2 = np.matmul(u1.T, sigma2)
    dW2 = dW2 + reg * W2

    #errors matrix of hidden layer and accumulated db1
    sigma1 = u1/z1 * np.matmul(sigma2, W2.T)
    db1 = sigma1.sum(axis = 0)
